...
sub readORFdataFiles
{
	...
	my ($orfLengthsFile, $orf8mersFile, $utrOffset6mersFile, ) = @_;
...
sub getFileFormats
{
	...
	** Required input files:
	...
	4 - ORF lengths (for ORFs matching 3\' UTRs in UTR_file):     file with Gene/UTR ID, species ID, length
	                                                              created by targetscan_count_offset_6mers.pl
...
sub getOffset6mer_weighted_contribution
{
	...
	if ($utrToOffset6merSites{$transcriptID}{$speciesID}{$miRNA_familyID} && $utrToOffset6merSites{$transcriptID}{$speciesID}{$miRNA_familyID} != -1)
	{	
		@offset6merSites = $utrToOffset6merSites{$transcriptID}{$speciesID}{$miRNA_familyID};
