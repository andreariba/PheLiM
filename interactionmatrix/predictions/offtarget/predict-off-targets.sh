...
echo "### OFF-TARGET PREDICTIONS ###"
if [ "$#" -ne "3" ]; then
	echo "One or more input files are missing.\nUsage: bash predict-off-targets.sh <siRNA fasta file (21nt)> <transcript UTR fasta file> <transcript ORF fasta file>\n"
	exit -1
else
	...
	perl targetscan_70_context_scores.pl sirnas_for_context_scores.txt UTR.txt targetscan_70_output.BL_PCT.txt ORF.length.txt ORF_8mer_counts.txt Targets.BL_PCT.context_scores.txt
        ...
        rm temp.txt 6mer.txt 7merA1.txt 7merm8.txt 8mer.txt

fi
echo "Output: offtarget_predictions.tab"
echo "### OFF-TARGET PREDICTIONS DONE ###"
